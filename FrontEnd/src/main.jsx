    ...
    <main className="app-shell">
      ...
      <div className="page-layout">
        <section className="category-section" id="categories">
          <div className="service-area"><p className="eyebrow">Service area</p><p>SPSR Nellore District, Andhra Pradesh</p>{mandals.map((mandal) => <strong key={mandal}>{mandal} · {mandalVillages[mandal].length} villages</strong>)}</div>
      ...
      <form onSubmit={handleSubmit}>
        {!generatedOtp && <>
          ...
          {mode === 'signup' && <>
            ...
            <label>Village<select required={availableVillages.length > 0} value={selectedVillage} disabled={!selectedMandal || availableVillages.length === 0} onChange={(event) => setSelectedVillage(event.target.value)}><option value="" disabled>{selectedMandal && availableVillages.length === 0 ? 'No village options available' : 'Select a village'}</option>{availableVillages.map((village) => <option key={village} value={village}>{village}</option>)}</select><ChevronDown className="select-icon" size={16} /></label>
          </>}
